...
select * from rule_master;

SET rule_id_completeness = (
    SELECT rule_id
    FROM rule_master
...
SET rule_id_phone = (
    SELECT rule_id
    FROM rule_master
    WHERE rule_name = 'VALIDITY_PHONE_FORMAT'
      AND is_active = TRUE
    ORDER BY rule_id DESC
    LIMIT 1
);

SET rule_id_date_format = (
    SELECT rule_id
    FROM rule_master
    WHERE rule_name = 'VALIDITY_DATE_FORMAT'
      AND is_active = TRUE
    ORDER BY rule_id DESC
    LIMIT 1
);
...
SET rule_id_positive = (
    SELECT rule_id
    FROM rule_master
    WHERE rule_name = 'VALIDITY_POSITIVE_NUMBER'
      AND is_active = TRUE
    ORDER BY rule_id DESC
    LIMIT 1
);

SET rule_id_non_negative = (
    SELECT rule_id
    FROM rule_master
    WHERE rule_name = 'VALIDITY_NON_NEGATIVE_NUMBER'
      AND is_active = TRUE
    ORDER BY rule_id DESC
    LIMIT 1
);

SET rule_id_allowed_values = (
    SELECT rule_id
    FROM rule_master
    WHERE rule_name = 'VALIDITY_ALLOWED_VALUES'
      AND is_active = TRUE
    ORDER BY rule_id DESC
    LIMIT 1
);

SET rule_id_date_range = (
    SELECT rule_id
    FROM rule_master
    WHERE rule_name = 'VALIDITY_DATE_RANGE'
      AND is_active = TRUE
    ORDER BY rule_id DESC
    LIMIT 1
);

SET rule_id_freshness = (
    SELECT rule_id
    FROM rule_master
    WHERE rule_name = 'FRESHNESS_DATA_RECENCY'
      AND is_active = TRUE
    ORDER BY rule_id DESC
    LIMIT 1
);
...
SET rule_id_fk = (
    SELECT rule_id
    FROM rule_master
    WHERE rule_name = 'CONSISTENCY_FOREIGN_KEY'
      AND is_active = TRUE
    ORDER BY rule_id DESC
    LIMIT 1
);
...
SET rule_id_country = (
    SELECT rule_id
    FROM rule_master
    WHERE rule_name = 'VALIDITY_COUNTRY_CODE'
      AND is_active = TRUE
    ORDER BY rule_id DESC
    LIMIT 1
);

SET rule_id_currency = (
    SELECT rule_id
    FROM rule_master
    WHERE rule_name = 'VALIDITY_CURRENCY_CODE'
      AND is_active = TRUE
    ORDER BY rule_id DESC
    LIMIT 1
);
