...
-- Link to Lineage Setup
USE DATABASE DATA_QUALITY_DB;
USE SCHEMA DQ_METRICS;
-- 1. Create the Lineage Table
CREATE TABLE IF NOT EXISTS DATA_LINEAGE (
    ID VARCHAR DEFAULT UUID_STRING(),
    UPSTREAM_DATABASE VARCHAR,
    UPSTREAM_SCHEMA VARCHAR,
    UPSTREAM_TABLE VARCHAR,
    DOWNSTREAM_DATABASE VARCHAR,
    DOWNSTREAM_SCHEMA VARCHAR,
    DOWNSTREAM_TABLE VARCHAR,
    CREATED_AT TIMESTAMP_LTZ DEFAULT CURRENT_TIMESTAMP()
);
-- 2. Clear existing sample data for STG_ACCOUNT to avoid duplicates
DELETE FROM DATA_LINEAGE 
WHERE (DOWNSTREAM_TABLE = 'STG_ACCOUNT' AND DOWNSTREAM_SCHEMA = 'BANKING_DW_BRONZE')
   OR (UPSTREAM_TABLE = 'STG_ACCOUNT' AND UPSTREAM_SCHEMA = 'BANKING_DW_BRONZE');
-- 3. Insert Sample Upstream Lineage (Source -> STG_ACCOUNT)
-- CRM System feeding into Stage
INSERT INTO DATA_LINEAGE (UPSTREAM_DATABASE, UPSTREAM_SCHEMA, UPSTREAM_TABLE, DOWNSTREAM_DATABASE, DOWNSTREAM_SCHEMA, DOWNSTREAM_TABLE)
VALUES 
('RAW_ZONE', 'CRM', 'ACCOUNTS_RAW', 'BANKING_DW', 'BANKING_DW_BRONZE', 'STG_ACCOUNT'),
('RAW_ZONE', 'CRM', 'CUSTOMERS_RAW', 'BANKING_DW', 'BANKING_DW_BRONZE', 'STG_ACCOUNT');
-- 4. Insert Sample Downstream Lineage (STG_ACCOUNT -> Target)
-- Stage feeding into Fact/Dimension tables
INSERT INTO DATA_LINEAGE (UPSTREAM_DATABASE, UPSTREAM_SCHEMA, UPSTREAM_TABLE, DOWNSTREAM_DATABASE, DOWNSTREAM_SCHEMA, DOWNSTREAM_TABLE)
VALUES 
('BANKING_DW', 'BANKING_DW_BRONZE', 'STG_ACCOUNT', 'BANKING_DW', 'BANKING_DW_SILVER', 'DIM_ACCOUNT'),
('BANKING_DW', 'BANKING_DW_BRONZE', 'STG_ACCOUNT', 'BANKING_DW', 'BANKING_DW_SILVER', 'FACT_TRANSACTIONS');
-- Verify the data
